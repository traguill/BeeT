...
BeeT_decorator * BeeT_Decorator_Init(BeeT_Serializer * data, BeeT_Blackboard * bb, int nodeId)
{
	...
	if (dec->varA)
	{
		...
		case BV_STRING:
		{
			...
			break;
		case BV_VECTOR2:
			dec->varB = BEET_malloc(sizeof(float2));
			*(float2*)dec->varB = BeeT_Serializer_GetFloat2(data, "var2");
			BEET_bool(*funcVector2[2])(BeeT_decorator*) = {&Pass_Vector2Equal, &Pass_Vector2NotEqual};
			dec->Pass = funcVector2[compareOption];
			break;
		case BV_VECTOR3:
			dec->varB = BEET_malloc(sizeof(float3));
			*(float3*)dec->varB = BeeT_Serializer_GetFloat3(data, "var2");
			BEET_bool(*funcVector3[2])(BeeT_decorator*) = { &Pass_Vector3Equal, &Pass_Vector3NotEqual };
			dec->Pass = funcVector3[compareOption];
			break;
		}
	}
...
{
	float2* varA = (float2*)dec->varA->data;
	float2* varB = (float2*)dec->varB;
	return (varA->x == varB->x && varA->y == varB->y) ? BEET_TRUE : BEET_FALSE;
}
BEET_bool Pass_Vector2NotEqual(BeeT_decorator * dec)
{
	float2* varA = (float2*)dec->varA->data;
	float2* varB = (float2*)dec->varB;
	return (varA->x == varB->x && varA->y == varB->y) ? BEET_FALSE : BEET_TRUE;
}
BEET_bool Pass_Vector3Equal(BeeT_decorator * dec)
{
	float3* varA = (float3*)dec->varA->data;
	float3* varB = (float3*)dec->varB;
	return (varA->x == varB->x && varA->y == varB->y && varA->z == varB->z) ? BEET_TRUE : BEET_FALSE;
}
BEET_bool Pass_Vector3NotEqual(BeeT_decorator * dec)
{
	float3* varA = (float3*)dec->varA->data;
	float3* varB = (float3*)dec->varB;
	return (varA->x == varB->x && varA->y == varB->y && varA->z == varB->z) ? BEET_FALSE : BEET_TRUE;
}
// ------------------------------------------------------------